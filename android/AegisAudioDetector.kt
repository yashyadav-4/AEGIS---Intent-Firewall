...
class AegisAudioDetector(private val context: Context) {
    ...
    private var wavlmInterpreter: Interpreter? = null
    private var wavlmAvailable = false
    ...
    fun analyze(waveform: FloatArray): DetectionResult {
        ...
        if (!wavlmAvailable) {
            loadWavLMInterpreter()
        }

        val wavlmScore = if (wavlmAvailable && wavlmInterpreter != null) {
            // TODO: Replace placeholder zeros with real 128-dim WavLM consistency features.
            val wavlmFeatures = FloatArray(128) { 0.0f }
            val wavlmScaled = applyScaler(wavlmFeatures, wavlmScaler)
            runInt8SingleOutput(wavlmInterpreter!!, wavlmScaled)
        } else {
            0.0f
        }

        val fusionInput = if (wavlmAvailable) {
            floatArrayOf(
            ...
            floatArrayOf(
                phaseScore, glottalScore, 0f,
                phaseScore * glottalScore, 0f, 0f
            )
        }
    ...
    private fun loadWavLMInterpreter() {
        if (wavlmInterpreter != null) return
        synchronized(this) {
            if (wavlmInterpreter == null) {
                try {
                    val interpreter = loadInterpreter("detector_wavlm.tflite", createOptions())
                    interpreter.allocateTensors()
                    wavlmInterpreter = interpreter
                    wavlmAvailable = true
                } catch (_: Exception) {
                    wavlmInterpreter = null
                    wavlmAvailable = false
                }
            }
        }
    }
