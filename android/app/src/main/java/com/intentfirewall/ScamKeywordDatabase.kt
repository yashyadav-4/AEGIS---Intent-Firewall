...
object ScamKeywordDatabase {
    ...
    private fun otp() = ScamCategoryDef(
        ...
        baseScore = 95,
        high = setOf("otp", "one time password", "verification code", "otp share karo", "share your otp", "give me your otp", "code batao", "code bhejo", "4 digit code", "6 digit code", "bank otp", "upi pin", "o.t.p", "otp number", "verification number", "otp aaya", "woh code", "wo code", "code aaya"),
        medium = setOf("verify karo", "account verify", "code aaya hoga", "sms mein code", "registration code", "token number", "security number", "confirm code", "activation code"),
        low = setOf("confirm karo", "verify your account", "security code", "code", "number batao", "pin"),
        hinglish = setOf("woh code batao", "bank ka code", "woh number share karo", "code aaya na", "otp dedo", "code dedo", "wo number batao", "code bata do"),
        evidenceBoostKeywords = setOf("bank", "rbi", "sbi", "hdfc", "icici", "paytm", "upi", "government", "telecom", "airtel", "jio", "bsnl"),
        ...
        groups = mapOf(
            ...
            "SHARE_REQUEST" to setOf("batao", "bhejo", "share", "send", "dedo", "bolo", "give", "give me", "provide", "provide me", "tell", "tell me")
        )
    )
    ...
    private fun kyc() = ScamCategoryDef(
        ...
        baseScore = 88,
        high = setOf("kyc update", "kyc expired", "kyc pending", "account band ho jayega", "account block", "aadhaar update", "pan update", "document submit", "kyc complete", "re-kyc", "video kyc", "kyc verify", "kyc link"),
        medium = setOf("kyc complete karo", "link expire", "72 ghante", "48 hours", "verify your kyc", "submit documents", "account suspend", "account deactivate", "document upload", "24 ghante mein"),
        ...
        evidenceBoostKeywords = setOf("band ho jayega", "block", "suspend", "deactivate", "72", "48", "24 hours", "aaj hi", "expire", "last date"),
    ...
    private fun lottery() = ScamCategoryDef(
        ...
        baseScore = 97,
        high = setOf("congratulations you won", "lucky draw", "prize money", "jackpot", "lottery winner", "claim your prize", "lakh won", "free iphone", "gift voucher won", "bumper prize", "mega prize", "winner declared", "you have been selected", "cash prize", "2 crore lottery", "50 lakh prize"),
        medium = setOf("selected ho gaye", "winner hai aap", "redeem karo", "claim karo", "prize claim", "reward collect", "prize release", "winning amount", "prize ka paisa"),
        ...
        evidenceBoostKeywords = setOf("processing fee", "tax pay karo", "courier charge", "registration fee", "small fee", "release charge", "claim karne ke liye paisa", "verification fee"),
    ...
    private fun job() = ScamCategoryDef(
        ...
        baseScore = 85,
        high = setOf("work from home", "part time job", "earn daily", "task complete karo", "like karo aur paisa lo", "youtube like job", "product rating job", "per task payment", "ghar baithe job", "online task", "telegram task", "hotel rating", "app rating job", "500 per like", "1000 per task", "daily withdrawal", "instant withdrawal task"),
        medium = setOf("ghar baithe kamao", "daily income", "flexible job", "online earning", "task based", "500 per day", "earn per click", "social media job", "data entry job", "typing job online", "survey job"),
        ...
        evidenceBoostKeywords = setOf("advance deposit", "registration fee", "wallet", "invest to unlock", "upgrade plan", "membership fee", "security deposit", "pehle deposit karo", "task unlock karne ke liye paisa"),
    ...
    private fun impersonation() = ScamCategoryDef(
        ...
        baseScore = 90,
        high = setOf("main bank se bol raha hoon", "cbi officer", "police se bol raha", "trai", "rbi officer", "income tax department", "main aapka dost hoon new number", "courier agent", "fedex parcel", "customs officer", "cyber crime department", "narcotics officer", "it officer", "enforcement directorate", "ed officer", "ncb officer"),
        medium = setOf("government official", "legal notice", "arrest warrant", "case filed against you", "customs duty", "parcel pakda gaya", "your number involved", "aapka number blacklist mein", "aapke naam complaint", "summons bheja hai"),
        low = setOf("official call", "legal matter", "notice bheja hai", "complaint hai aapke against", "verification call"),
        hinglish = setOf("main police se hoon", "aapke naam arrest warrant hai", "ye mera naya number hai paise bhejo", "cbi ne case kiya hai", "aapka number illegal activity mein use hua", "aapke account se fraud hua hai"),
        evidenceBoostKeywords = setOf("arrest", "warrant", "case", "fine", "pay now", "jail", "legal action", "court", "giraftari", "fir", "chalaan"),
        requireAllGroups = listOf("AUTHORITY_CLAIM_TOKEN", "THREAT_OR_DEMAND"),
        requireAnyGroup = emptyList(),
        groups = mapOf(
            "AUTHORITY_CLAIM_TOKEN" to setOf("officer", "police", "cbi", "rbi", "trai", "government", "department", "court", "bank", "customs", "narcotics", "ed", "ncb"),
            "THREAT_OR_DEMAND" to setOf("arrest", "warrant", "case", "fine", "pay", "legal", "action", "giraftari", "send money", "account freeze")
        )
    )
    ...
    private fun romance() = ScamCategoryDef(
        ...
        baseScore = 80,
        high = setOf("video call record kar liya", "screenshot le liya", "contacts ko bhej dunga", "blackmail", "intimate video", "viral kar dunga", "pay karo warna", "video leak karunga", "photo bhej dunga", "family ko dikhaunga", "nude video", "private video", "aapki photo mere paas", "morphed photo"),
        medium = setOf("meri photo mat share karna", "hamari baat private hai", "paisa bhejo", "army officer hu", "video hai mere paas", "screenshot liya", "foreign mein hoon", "dollar mein kamata hoon"),
        ...
        evidenceBoostKeywords = setOf("pay", "send money", "paisa bhejo", "family", "contacts", "warna", "otherwise", "50000", "100000", "transfer karo"),
        requireAllGroups = listOf("BLACKMAIL_TOKEN", "PAYMENT_THREAT"),
        requireAnyGroup = emptyList(),
        groups = mapOf(
            "BLACKMAIL_TOKEN" to setOf("video", "photo", "screenshot", "record", "viral", "leak", "nude"),
            "PAYMENT_THREAT" to setOf("pay", "send", "transfer", "paisa", "bhejo", "warna", "otherwise")
        )
    )
    ...
    private fun techSupport() = ScamCategoryDef(
        ...
        baseScore = 83,
        high = setOf("anydesk install karo", "teamviewer", "remote access", "screen share karo", "virus detected", "your computer hacked", "refund process", "amazon refund", "microsoft support", "quicksupport", "rustdesk", "ultraviewer", "airdroid", "remote desktop", "screen mirroring karo", "hamein access do"),
        medium = setOf("technical issue", "your account compromised", "security breach", "support agent", "helpline number", "customer care", "device infected", "malware found", "suspicious activity", "account hack ho gaya", "data breach"),
    ...
    private fun courier() = ScamCategoryDef(
        ...
        baseScore = 86,
        high = setOf("parcel pakda gaya", "customs duty pay karo", "courier hold", "illegal item parcel", "fedex parcel", "dhl notice", "drug parcel aapke naam", "courier agent", "parcel seized", "narcotics in parcel", "aadhaar card parcel", "sim card parcel", "parcel mein drugs mili", "illegal courier aapke naam"),
        medium = setOf("delivery pending", "parcel release karne ke liye", "fine bharo", "package seized", "customs clearance", "delivery agent", "parcel stuck customs mein", "courier pakda gaya"),
        ...
        evidenceBoostKeywords = setOf("drug", "illegal", "narcotic", "seized", "fine", "duty", "arrested", "case", "police", "narcotics department"),
    ...
    private fun utility() = ScamCategoryDef(
        ...
        baseScore = 89,
        high = setOf("bijli kategi aaj raat", "electricity disconnected", "bill due aaj", "last warning bijli", "pay now warna cut", "bescom", "mseb", "bses notice", "electricity cut tonight", "power disconnection", "meter disconnect notice", "gas connection cut", "broadband disconnect notice", "bijli vibhag se bol raha hoon"),
        medium = setOf("bill pending", "overdue bill", "disconnection notice", "pay immediately", "link se pay karo", "meter disconnect", "supply cut", "outstanding bill", "bill baaki hai"),
        ...
        evidenceBoostKeywords = setOf("aaj raat", "tonight", "2 hours", "immediately", "link", "pay now", "cut ho jayega", "abhi bharo", "online pay karo"),
