...
def yamnet(features, params):
  """Define the core YAMNet model in Keras using MobileNetV3-Small."""
  net = layers.Reshape(
      (params.patch_frames, params.patch_bands, 1),
      input_shape=(params.patch_frames, params.patch_bands))(features)
  
  # MobileNetV3 expects 3 channels. Log-mel is 1 channel, so we duplicate it.
  net_3ch = layers.Concatenate(axis=-1)([net, net, net])
  
  base_model = tf.keras.applications.MobileNetV3Small(
      input_shape=(params.patch_frames, params.patch_bands, 3),
      include_top=False,
      weights=None
  )
  
  x = base_model(net_3ch)
  embeddings = layers.GlobalAveragePooling2D()(x)
  
  # Binary classification: 0=real, 1=synthetic
  predictions = layers.Dense(1, activation='sigmoid')(embeddings)
